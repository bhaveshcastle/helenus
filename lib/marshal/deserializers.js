...
Deserializers.decodeCollection = function (deserializers) {
  var pos = 0, result, len, buf = new Buffer(deserializers, 'binary');
  console.log("buf", buf.toString('hex'));
  total = buf.readUInt16BE(pos);
  pos += 2;
  while(pos < buf.length) {
    len = buf.readUInt16BE(pos);
    pos += 2;
    result.push(deserializers.slice(pos, len + pos));
    // console.log("total = ", total, "; len = ", len, "; pos = ", pos);
    pos += len;
  }
  return result;
/*  console.log("deserializers.length = ", deserializers.length);
  if(deserializers === null || deserializers === undefined) {
    return null;
  }

  var buf = new Buffer(deserializers, 'binary'),
  ...
  while(pos < buf.length){
    ...
    else if(i == 0 && deserializers.length == 1 || 
            i == 1 && deserializers.length == 2) {
    ...
    pos += len;
    console.log("pos = ", pos, "; vals = ", vals, "; keys = ", keys);
  }
